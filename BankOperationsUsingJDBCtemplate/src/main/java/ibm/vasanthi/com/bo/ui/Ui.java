...
public class Ui {
	...
	for(;;)
	{
		...
		int num = sc.nextInt();
		if (num == 1) {
			List<CustomerDetails> customerDetails = service.displayTable();
			...
			if (isValid && amount1 > 0) {
				...
				if (service.checkAccount(anotherAccountNo)) {
					amount2 = service.checkBalance(accountNo);
					System.out.println("Enter the ammount you want to transfer:");
					amountToTransfer = sc.nextInt();
					if (amount1 > amountToTransfer) {
						service.fundTransfer(accountNo, anotherAccountNo, amountToTransfer,amount1,amount2);
						transaction.setTransAccount(accountNo);
